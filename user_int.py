...
class App(tk.Tk):
    ...
    def import_folder(self):
        '''
        Asks a file path from the user and imports image data from it. Checks if location and label
        data are provided and imports any provided data. Displays strings to indicate whether or not 
        location and label data are available in the data input window and shows the names of any 
        imported location files. Transforms the inport data to a csv data format. 
        This function is called when the "Input folder" button in the pop-up window is clicked.
        
        Returns
        ------------
        import_df:  A dataframe containing the image data and (if inputted by the user) the location 
                    and label data in csv format
        ...
        if folder_path:
            ...
            import_df = self.import_images(images_folder)
            
            if os.path.exists(locations_folder):
                location_label, import_df = self.import_locations(images_folder, locations_folder, import_df)
            else:
            ...
            if os.path.exists(labels_folder):
                label_label, import_df = self.import_labels(images_folder, labels_folder, import_df)
            else:
    ...
    def import_images(self, images_folder):
        '''
        Performs checks on the image folder and removes any .DS_Store files. Extracts 
        data from the image folder returnes it as a dataframe. This function is called 
        in the import_folder() method (see above).

        Parameters
        ---------------
        images_folder: The folder containing the input images

        Returns:
        ---------------
        import_df: A dataframe containing the image data extracted from the image folder
        '''
    ...
    def import_locations(self, images_folder, locations_folder, import_df):
        '''
        Performs checks on the locations folder and removes any .DS_Store files. Extracts 
        data from the location folder and adds the data to the import dataframe. This function 
        is called in the import_folder() method if location data is available (see above).

        Parameters
        ---------------
        images_folder: The folder containing the input images
        locations_folder: The folder containing the input locations 

        Returns:
        ---------------
        location_label: A string describing whether or not location data is available. This
                        string will later be shown in the window where the user inputs data
                        (see import_folder() above).
        import_df:      A dataframe containing the image data extracted from the image folder
                        in addition to the previously imported data
        '''
        ...
        return location_label, import_df

    def import_labels(self, images_folder, labels_folder, import_df):
        '''
        Performs checks on the labels folder and removes any .DS_Store files. Extracts 
        data from the label folder and adds the data to the import dataframe. This function 
        is called in the import_folder() method if label data is available (see above).

        Parameters
        ---------------
        images_folder: The folder containing the input images
        labels_folder: The folder containing the input labels 

        Returns:
        ---------------
        label_label: A string describing whether or not label data is available. This
                    string will later be shown in the window where the user inputs data
                    (see import_folder() above).
        import_df:  A dataframe containing the image data extracted from the image folder
                    in addition to the previously imported data
